...
import { Box, Chip, Collapse, Divider, Grid, IconButton, Stack, Tooltip, Typography } from "@mui/material";
import {
  AlertCircle,
  AlertTriangle,
  BarChart3,
...
.db-tab-on {
  ...
  box-shadow:
    ...
    0 5px 16px rgba(34, 211, 238, 0.16);
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (uid) {
      const p = parseInt(uid, 10);
      if (!isNaN(p)) setViewUserId(p);
    }
  }, [searchParams]);
  ...
  const handleUserSelect = (user: User) => {
    ...
    p.set("view_user_id", String(user.id));
  ...
  const ENTITY_COLORS: Record<string, string> = {
    ...
    shipment: C.pine,
    invoice: C.gold,
    sales_order: C.saffron,
    purchase_order: C.rust,
  };
  ...
  const kpiCards = dm
    ? [
        {
          title: "Customers",
          value: dm.customer_metrics.total,
          sub: `${dm.customer_metrics.active} active`,
          color: C.azure,
          spark: getTrend("customer"),
          icon: Users,
        },
        {
          title: "Vendors",
          value: dm.vendor_metrics.total,
          sub: `${dm.vendor_metrics.active} active`,
          color: C.plum,
          spark: getTrend("vendor"),
          icon: ShoppingCart,
        },
        {
          title: "Inventory Items",
          value: dm.item_metrics.total,
          sub: `${dm.item_metrics.item_groups} groups`,
          color: C.teal,
          spark: getTrend("item"),
          icon: Package,
        },
        {
          title: "Shipments",
          value: dm.shipment_metrics.total,
          sub: `${dm.shipment_metrics.in_transit} in transit`,
          color: C.pine,
          spark: getTrend("shipment"),
          icon: Truck,
        },
        {
          title: "Revenue",
          value: `₹${(dm.invoice_metrics.total_amount / 100000).toFixed(1)}L`,
          sub: `${dm.invoice_metrics.overdue_count} overdue`,
          color: C.gold,
          spark: getTrend("invoice"),
          icon: DollarSign,
        },
        {
          title: "Sales Orders",
          value: dm.sales_order_metrics.total,
          sub: `${dm.sales_order_metrics.pending_count} pending`,
          color: C.saffron,
          spark: getTrend("sales_order"),
          icon: BarChart3,
        },
        {
          title: "Purchase Orders",
          value: dm.purchase_order_metrics.total,
          sub: `${dm.purchase_order_metrics.completed_count} done`,
          color: C.rust,
          spark: getTrend("purchase_order"),
          icon: FileText,
        },
        {
          title: "Packages",
          value: dm.package_metrics.total,
          sub: `${dm.package_metrics.delivered_count} delivered`,
          color: C.inkMid,
          spark: getTrend("shipment"),
          icon: Layers,
        },
  ...
  const SectionHeading = ({ children, delay = "0s" }: { children: React.ReactNode; delay?: string }) => (
    <Typography
      className="db-left"
      sx={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 17, color: C.ink, mb: 2, mt: 0.5, animationDelay: delay }}
    >
      {children}
    </Typography>
  );
  ...
  return (
    <Box
      className="db-root"
      sx={{ fontFamily: BODY, color: C.ink, bgcolor: C.bg, minHeight: "100vh", p: { xs: 2, sm: 3 } }}
    >
      <style>{GLOBAL_CSS}</style>
      <BBLoader
        enabled={loading || statsLoading || loadingMetrics || loadingActivity || loadingStock || loadingTrends}
      />

      {/* ══ HEADER ═══════════════════════════════════════════════════════ */}
      <div
        className="db-header db-fade"
        style={{
          marginBottom: 24,
          padding: "28px 32px",
          animationDelay: "0s",
          ...
          overflow: "visible",
          background: "linear-gradient(135deg, #8B5CF6 0%, #22D3EE 100%)",
          boxShadow: "0 18px 45px rgba(139, 92, 246, 0.24), 0 10px 30px rgba(34, 211, 238, 0.18)",
        ...
        >
          <div>
            ...
            <div
              style={{
                ...
                lineHeight: 1.15,
                fontFamily: DISPLAY,
                textShadow: "0 4px 18px rgba(30, 20, 70, 0.18)",
              }}
            >
              Business Overview
            </div>
          ...
          >
              ...
              <div style={{ position: "relative" }}>
                <button
                  ...
                  style={{
                    ...
                    boxShadow: "0 6px 18px rgba(67, 44, 150, 0.12)",
            ...
            <Tooltip title="Refresh all metrics">
              <button
                onClick={() => refreshDashboard(viewUserId)}
                disabled={refreshing}
                style={{
                  background: "rgba(255,255,255,0.16)",
                  border: "1px solid rgba(255,255,255,0.32)",
      ...
      {(lowStock > 0 || outOfStock > 0) && (
        ...
        >
          ...
          <button
            className="db-btn"
            style={{ borderColor: `${C.gold}55`, color: C.gold, fontSize: 12, padding: "7px 14px" }}
      ...
      <Stack direction="row" gap={1} mb={3} flexWrap="wrap">
        {(
          ...
          <button
            key={id}
            className={`db-tab db-fade ${activeTab === id ? "db-tab-on" : "db-tab-off"}`}
          ...
          <Grid container spacing={2} sx={{ mb: 3 }}>
            {kpiCards.map(({ title, value, sub, color, spark, icon: Icon }, i) => (
              <Grid key={title} size={{ xs: 12, sm: 6, lg: 3 }}>
                <Box
                  className="db-card db-kpi db-fade"
                  sx={{ p: 2.5, animationDelay: `${0.14 + i * 0.04}s`, "--kpi-bg": `${color}06` } as any}
                >
                  ...
                  <Collapse in={allExpanded} timeout="auto" unmountOnExit>
                    <Box sx={{ mt: 2, pt: 2, borderTop: `1px solid ${C.border}` }}>
                      ...
                      <Typography sx={{ fontSize: 10, color: C.subtle, textAlign: "right", mt: 0.5, fontFamily: MONO }}>
                        30-day trend
                      </Typography>
                    </Box>
                  </Collapse>
                  ...
                  {[
                    ...
                    <Box key={label} mb={2.2}>
                      <Stack direction="row" justifyContent="space-between" mb={0.8}>
                        ...
                        <Typography className="db-num" sx={{ fontSize: 12, fontWeight: 700, color: C.ink }}>
                          {val.toLocaleString()}
                        </Typography>
                      </Stack>
                  ...
                  >
                    ...
                    <Box>
                      ...
                      <Typography className="db-num" sx={{ fontSize: 18, fontWeight: 700, color: C.teal }}>
                        ...
                        <Typography component="span" sx={{ fontSize: 12, color: C.muted, ml: 0.5 }}>
                          days
                        </Typography>
                      </Typography>
                    </Box>
              ...
              <Grid size={{ xs: 12, md: 4 }}>
                <Box className="db-card db-fade" sx={{ p: 3, height: "100%", animationDelay: "0.48s" }}>
                  ...
                  {[
                    {
                      label: "Total Revenue",
                      val: `₹${(dm.invoice_metrics.total_amount / 100000).toFixed(1)}L`,
                      color: C.ink,
                    },
                    {
                      label: "Outstanding",
                      val: `₹${(dm.invoice_metrics.outstanding_amount / 1000).toFixed(0)}K`,
                      color: C.gold,
                    },
                    ...
                    <Stack
                      ...
                      direction="row"
                      justifyContent="space-between"
                      sx={{ py: 1, borderTop: `1px solid ${C.border}` }}
                    >
                      <Typography sx={{ fontSize: 12, color: C.muted }}>{label}</Typography>
                      <Typography className="db-num" sx={{ fontSize: 13, fontWeight: 700, color }}>
                        {val}
                      </Typography>
                    </Stack>
                  ))}
                ...
                <Box className="db-card db-fade" sx={{ p: 3, height: "100%", animationDelay: "0.52s" }}>
                  ...
                  <Grid container spacing={1.5} sx={{ mb: 2 }}>
                    {[
                      {
                        label: "Sales",
                        total: dm.sales_order_metrics.total,
                        done: dm.sales_order_metrics.completed_count,
                      ...
                      {
                        label: "Purchase",
                        total: dm.purchase_order_metrics.total,
                        done: dm.purchase_order_metrics.completed_count,
                      ...
                      <Grid key={label} size={6}>
                        <Box sx={{ background: `${color}0A`, border: `1px solid ${color}20`, borderRadius: 3, p: 2 }}>
                          ...
                          <Typography className="db-num" sx={{ fontSize: 24, fontWeight: 700, color, mb: 1 }}>
                  ...
                  >
                    <Box>
                      ...
                      <Typography className="db-num" sx={{ fontSize: 20, fontWeight: 700, color: C.pine }}>
                        {dm.package_metrics.delivered_count.toLocaleString()}
                      </Typography>
                    </Box>
                    <span
                      style={{
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      <tbody>
                        {trends.data.slice(0, 10).map((tr, idx) => (
                          ...
                          >
                            <td style={{ padding: "11px 16px", fontSize: 12, color: C.inkMid, fontFamily: MONO }}>
                  ...
                  {[
                    ...
                    <Grid key={label} size={{ xs: 6, sm: 4, md: "auto" }}>
                      <Box className="db-card db-fade" sx={{ p: 2.5, animationDelay: `${0.06 * i}s` }}>
                        ...
                        <Typography className="db-num" sx={{ fontSize: 24, fontWeight: 700, color, mt: 1.5, mb: 0.3 }}>
                          {val?.toLocaleString()}
                        </Typography>
                        <Typography
                          sx={{
                ...
                <>
                          ...
                          {dashStocks.map((item: DashboardStockItem, idx) => (
                            ...
                            >
                              ...
                              <td style={{ padding: "14px 16px", textAlign: "center" }}>
                                <Typography className="db-num" sx={{ fontWeight: 700, fontSize: 14, color: C.ink }}>
                                  {item.current_stock}
                                </Typography>
                              </td>
                              <td style={{ padding: "14px 16px", textAlign: "center" }}>
                                <Typography className="db-num" sx={{ fontSize: 13, color: C.muted }}>
                              ...
                              <td style={{ padding: "14px 16px", textAlign: "center" }}>
                                <span
                                  style={{
                                    background: item.reserved_stock > 0 ? C.goldLt : C.bg,
                                    color: item.reserved_stock > 0 ? C.gold : C.subtle,
                                    padding: "3px 10px",
                                    borderRadius: 99,
                                    fontSize: 11,
                                    fontWeight: 700,
                              ...
                              <td style={{ padding: "14px 16px", textAlign: "center" }}>
                                <Typography className="db-num" sx={{ fontSize: 12, color: C.inkMid }}>
                                  {item.purchased_stock.toLocaleString()}
                                </Typography>
                              </td>
                              ...
                              <td style={{ padding: "14px 16px", textAlign: "center" }}>
                                <Typography className="db-num" sx={{ fontSize: 13, fontWeight: 600, color: C.inkMid }}>
                                  ₹{item.average_cost.toLocaleString()}
                                </Typography>
                              </td>
                              <td style={{ padding: "14px 16px", textAlign: "center" }}>
                  ...
                  <Grid container spacing={2}>
                    {dashStocks.map((item: DashboardStockItem, idx) => {
                      ...
                      const util =
                        item.current_stock > 0 ? Math.round((item.reserved_stock / item.current_stock) * 100) : 0;
                      return (
                        <Grid key={`${item.product_id}-card-${idx}`} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
                          <Box className="db-card db-fade" sx={{ p: 2.5, animationDelay: `${0.035 * idx}s` }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1.5}>
                              <Box>
                                <Typography sx={{ fontWeight: 700, fontSize: 13, color: C.ink, mb: 0.3 }}>
                                  {item.product_name}
                                </Typography>
                                <Typography className="db-num" sx={{ fontSize: 10, color: C.subtle }}>
                                  {item.product_id}
                                </Typography>
                              </Box>
                              <StockChip status={item.status} />
                              ...
                              {[
                                ...
                                <Grid key={label} size={6}>
                                  <Box sx={{ bgcolor: C.bg, borderRadius: 2, p: 1.2 }}>
                                    ...
                                    <Typography className="db-num" sx={{ fontSize: 18, fontWeight: 700, color }}>
                                      {val.toLocaleString()}
                                    </Typography>
                                  </Box>
                                </Grid>
                            ...
                            <Stack direction="row" justifyContent="space-between" mb={0.5}>
                              ...
                              <Typography className="db-num" sx={{ fontSize: 10, fontWeight: 700, color: sc }}>
                                {util}%
                              </Typography>
                            </Stack>
            ...
            <>
              ...
              <Grid container spacing={2} sx={{ mb: 3 }}>
                {[
                  ...
                  {
                    label: "Sales Orders",
                    val: activitySummary.created_sales_orders_today,
                    color: C.pine,
                    icon: BarChart3,
                  },
                  {
                    label: "Purchase Orders",
                    val: activitySummary.created_purchase_orders_today,
                    color: C.gold,
                    icon: FileText,
                  },
                  ...
                  <Grid key={label} size={{ xs: 6, sm: 4, md: 3, lg: "auto" }}>
                    <Box className="db-card db-fade" sx={{ p: 2.5, animationDelay: `${0.05 * i}s` }}>
                      <IconBox Icon={Icon} color={color} size={36} />
                      <Typography className="db-num" sx={{ fontSize: 28, fontWeight: 700, color, mt: 1.5, mb: 0.3 }}>
                        {val}
                      </Typography>
                      <Typography
                        sx={{
                ...
                {[
                  {
                    ...
                    color: C.azure,
                    rows: [
                      ["Total", dm.customer_metrics.total],
                      ["Active", dm.customer_metrics.active],
                      ["Inactive", dm.customer_metrics.inactive],
                      ["New Today", dm.customer_metrics.created_today],
                    ],
                  },
                  {
                    title: "Packages",
                    ...
                    color: C.teal,
                    rows: [
                      ["Total", dm.package_metrics.total],
                      ["Delivered", dm.package_metrics.delivered_count],
                      ["In Transit", dm.package_metrics.in_transit_count],
                      ["Pending", dm.package_metrics.pending_count],
                    ],
                  },
                  {
                    title: "Items",
                    ...
                    color: C.plum,
                    rows: [
                      ["Total Items", dm.item_metrics.total],
                      ["Total Stock", dm.item_metrics.total_stock],
                      ["Low Stock", dm.item_metrics.low_stock_items],
                      ["Out of Stock", dm.item_metrics.out_of_stock_items],
                    ],
                  },
                  {
                    title: "Vendors",
                    ...
                    color: C.gold,
                    rows: [
                      ["Total", dm.vendor_metrics.total],
                      ["Active", dm.vendor_metrics.active],
                      ["Inactive", dm.vendor_metrics.inactive],
                      ["New Today", dm.vendor_metrics.created_today],
                    ],
                  },
                    ...
                    <Box className="db-card db-fade" sx={{ p: 2.5, animationDelay: `${0.07 * i}s` }}>
                      ...
                      {rows.map(([label, val]) => (
                        <Stack
                          ...
                          direction="row"
                          justifyContent="space-between"
                          sx={{ py: 1.1, borderTop: `1px solid ${C.border}` }}
                        >
                          <Typography sx={{ fontSize: 12, color: C.muted }}>{label}</Typography>
                          <Typography className="db-num" sx={{ fontSize: 14, fontWeight: 700, color: C.ink }}>
            ...
            <>
              <Stack direction="row" spacing={1} flexWrap="wrap" alignItems="center" sx={{ mb: 2 }}>
                <button
                  ...
                  style={{
                    border: "none",
                    background: summaryMode === "all" ? "linear-gradient(135deg, #8B5CF6 0%, #22D3EE 100%)" : "#FFFFFF",
                    color: summaryMode === "all" ? "#FFFFFF" : "#64748B",
                    boxShadow:
                      summaryMode === "all"
                        ? "0 8px 22px rgba(139,92,246,.22), 0 4px 14px rgba(34,211,238,.16)"
                ...
                <button
                  ...
                  style={{
                    border: "none",
                    background: summaryMode === "raw" ? "linear-gradient(135deg, #8B5CF6 0%, #22D3EE 100%)" : "#FFFFFF",
                    color: summaryMode === "raw" ? "#FFFFFF" : "#64748B",
                    boxShadow:
                      summaryMode === "raw"
                        ? "0 8px 22px rgba(139,92,246,.22), 0 4px 14px rgba(34,211,238,.16)"
                        : "0 3px 12px rgba(15,23,42,.05)",
                ...
                >
                  ...
                  >
                    {summaryMode === "raw" ? "raw material" : "all"}
                  </Box>{" "}
                  stock summary.
                </Typography>
              </Stack>
                  ...
                  {[
                    {
                      ...
                      val: summaryMeta.total_stock_value,
                      color: C.saffron,
                      icon: DollarSign,
                      fmt: (v: number) => `₹${(v / 100000).toFixed(1)}L`,
                    },
                    {
                      label: "Total Sold Value",
                      val: summaryMeta.total_sold_product_value,
                      color: C.rust,
                      icon: BarChart3,
                      fmt: (v: number) => `₹${(v / 100000).toFixed(1)}L`,
                    },
                    {
                      ...
                      val: summaryStocks.reduce((s, x) => s + x.current_stock, 0),
                      color: C.pine,
                      icon: Package,
                    ...
                    },
                    {
                      ...
                      val: summaryStocks.reduce((s, x) => s + x.available_stock, 0),
                      color: C.teal,
                      icon: CheckCircle,
                      fmt: (v: number) => v.toLocaleString(),
                    },
              ...
              {summaryStocks.length === 0 ? (
                ...
                <>
                  <SectionHeading>
                    {summaryMode === "raw" ? "Raw Material Stock" : "Variant-Level Stock"} — {summaryStocks.length}{" "}
                    {summaryMode === "raw" ? "items" : "variants"}
                  </SectionHeading>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                      ...
                      <tbody>
                        {summaryStocks.map((stock: StockSummaryItem, idx) => (
                          <tr
                            key={`${stock.product_id}-${stock.sku}-${idx}`}
                            className="db-tr"
                            style={{
                              borderBottom: `1px solid ${C.border}`,
                              background: idx % 2 === 0 ? C.surface : C.bg,
                          ...
                          >
                            ...
                            <td style={{ padding: "13px 16px", textAlign: "center" }}>
                              <Typography className="db-num" sx={{ fontWeight: 700, fontSize: 13, color: C.azure }}>
                                {stock.current_stock}
                              </Typography>
                            </td>
                            <td style={{ padding: "13px 16px", textAlign: "center" }}>
                            ...
                            <td style={{ padding: "13px 16px", textAlign: "center" }}>
                              <Typography
                                className="db-num"
                                sx={{ fontSize: 12, color: stock.reserved_stock > 0 ? C.gold : C.subtle }}
                              >
                            ...
                            <td style={{ padding: "13px 16px", textAlign: "center" }}>
                              <Typography className="db-num" sx={{ fontSize: 12, color: C.inkMid }}>
                                {stock.purchased_total.toLocaleString()}
                              </Typography>
                            </td>
                            {/* sold_total from /api/stock/summary */}
                            <td style={{ padding: "13px 16px", textAlign: "center" }}>
                              <Typography className="db-num" sx={{ fontSize: 12, fontWeight: 600, color: C.rust }}>
                                {stock.sold_total.toLocaleString()}
                              </Typography>
                            </td>
                            <td style={{ padding: "13px 16px", textAlign: "center" }}>
                              <Typography className="db-num" sx={{ fontSize: 12, color: C.muted }}>
                  ...
                  <Grid container spacing={2}>
                    {summaryStocks.map((stock: StockSummaryItem, idx) => {
                      ...
                      const pct = stock.purchased_total > 0 ? (stock.available_stock / stock.purchased_total) * 100 : 0;
                      ...
                      return (
                        <Grid key={`${stock.product_id}-${stock.sku}`} size={{ xs: 12, sm: 6, lg: 3 }}>
                          <Box className="db-card db-fade" sx={{ p: 2.5, animationDelay: `${0.035 * idx}s` }}>
                            <Stack direction="row" justifyContent="space-between" alignItems="flex-start" mb={1.5}>
                              <Box>
                                <Typography sx={{ fontWeight: 700, fontSize: 12, color: C.ink, mb: 0.2 }}>
                                  {stock.product_name}
                                </Typography>
                                <Typography className="db-num" sx={{ fontSize: 10, color: C.subtle }}>
                                  {stock.sku}
                              ...
                              {[
                                ...
                                <Grid key={label} size={6}>
                                  <Box sx={{ bgcolor: C.bg, borderRadius: 2, p: 1, textAlign: "center" }}>
                                    <Typography sx={{ fontSize: 9, color: C.subtle, fontWeight: 700, mb: 0.2 }}>
                                      {label}
                                    </Typography>
                                    <Typography className="db-num" sx={{ fontSize: 15, fontWeight: 700, color }}>
                                      {val.toLocaleString()}
                                    </Typography>
                                  </Box>
                                </Grid>
                            ...
                            <Stack direction="row" justifyContent="space-between" mb={0.5}>
                              ...
                              <Typography className="db-num" sx={{ fontSize: 11, color: C.inkMid }}>
                                ₹{stock.average_cost.toLocaleString()}
                              </Typography>
                            </Stack>
                            <Stack direction="row" justifyContent="space-between" mb={1}>
                              <Typography sx={{ fontSize: 10, color: C.muted }}>Availability</Typography>
                              <Typography className="db-num" sx={{ fontSize: 10, fontWeight: 700, color: col }}>
                                {pct.toFixed(0)}%
                              </Typography>
                            </Stack>
      ...
      >
        ...
        <Typography sx={{ fontSize: 11, color: C.subtle }}>Business Intelligence Dashboard</Typography>
      </Box>
    </Box>
  );
};
