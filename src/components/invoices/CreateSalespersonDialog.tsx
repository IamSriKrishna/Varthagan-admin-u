...
export const CreateSalespersonDialog: React.FC<CreateSalespersonDialogProps> = ({
  ...
  const handleSubmit = async (values: any, { setSubmitting }: any) => {
    try {
      ...
      const response = await appFetch(`${apiDomain}${salespersons.postSalesperson}`, {
        method: "POST",
        headers: {
      ...
      if (response.ok && result) {
        ...
        showToastMessage(result.message || result.error || "Failed to create salesperson", "error");
      }
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ fontWeight: 600, fontSize: "1.25rem" }}>Create New Salesperson</DialogTitle>
